      ...
      [](const output_wrapper ow, const int n_jets = 1, double beta = 0, double symmetry_cut = 0.1,
        ...
        for (unsigned int i = 0; i < css.size(); i++){  // iterate through events
          ...
          for (unsigned int j = 0; j < jets.size(); j++){
            ...
            auto substruct = soft.structure_of<fastjet::contrib::SoftDrop>();
            jet_groomed_delta_R.push_back(substruct.delta_R());
            jet_groomed_symmetry.push_back(substruct.symmetry());
